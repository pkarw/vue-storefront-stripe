<template>
  ...
  <div class="mb15 mt20 develodesign-stripe-stripe-container">
    <h4 class="mt0">
      <label for="develodesign-stripe-stripe-card-element">
    ...
    <div class="bg-cl-secondary px20 py20">
      <form action="" id="payment-form">
        <div class="form-row">
          ...
          <div id="develodesign-stripe-stripe-card-element">
            &nbsp;
            <!-- A Stripe Element will be inserted here. -->
          </div>

          <!-- Used to display Element errors. -->
          <div id="develodesign-stripe-stripe-card-errors" role="alert">
            &nbsp;
          </div>
        </div>
      </form>
...
<style scoped>
  ...
  .develodesign-stripe-stripe-container label {
    font-weight: 500;
    font-size: 14px;
    display: block;
    margin-bottom: 8px;
  ...
  #develodesign-stripe-stripe-card-errors {
    margin: 8px auto 0;
    color: #fa755a;
  }
  .develodesign-stripe-stripe-container .StripeElement {
  ...
  .develodesign-stripe-stripe-container .StripeElement--focus {
    box-shadow: 0 1px 3px 0 #cfd7df;
  }
  .develodesign-stripe-stripe-container .StripeElement--invalid {
    border-color: #fa755a;
  }
  .develodesign-stripe-stripe-container .StripeElement--webkit-autofill {
...
export default {
  name: 'DevelodesignStripePayment',
  data () {
    return {
      dd_stripe: {
        instance: null,
  ...
  methods: {
    ...
    createElements () {
      ...
      this.dd_stripe.card.mount('#develodesign-stripe-stripe-card-element')
    },
    bindEventListeners () {
    ...
    onStripeCardChange (event) {
      let displayError = document.getElementById('develodesign-stripe-stripe-card-errors')
    ...
    processStripeForm () {
      ...
      this.dd_stripe.instance.createToken(this.dd_stripe.card).then(function (result) {
        if (result.error) {
          ...
          let errorElement = document.getElementById('develodesign-stripe-stripe-card-errors')
